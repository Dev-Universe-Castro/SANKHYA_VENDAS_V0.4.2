  ...
  return (
    <>
      <div className="bg-white rounded-lg shadow-sm border h-full flex flex-col overflow-hidden">
        <div className="border-b p-2 sm:p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0 flex-shrink-0">
          <div className="flex items-center gap-2 sm:gap-4 w-full sm:w-auto">
            {visualizacao === 'calendario' && (
              <>
                <Button variant="outline" size="sm" onClick={today} className="text-xs sm:text-sm">
                  Hoje
                </Button>
                <div className="flex items-center gap-1 sm:gap-2">
                  <Button variant="ghost" size="icon" onClick={previousMonth} className="h-8 w-8 sm:h-10 sm:w-10">
            ...
            {visualizacao === 'lista' && (
              <h2 className="text-sm sm:text-xl font-semibold">Todas as Tarefas</h2>
            )}
          </div>
          <div className="flex items-center gap-1 sm:gap-2 w-full sm:w-auto overflow-x-auto">
            ...
            >
              ...
              <span className="hidden sm:inline">Inativos</span> ({eventosInativos.length})
            </Button>
            <Button onClick={abrirModalNovaAtividade} size="sm" className="text-xs whitespace-nowrap">
        ...
        {visualizacao === 'calendario' ? (
          <div className="flex-1 p-1 sm:p-4 flex flex-col overflow-hidden min-h-0">
            <div className="grid grid-cols-7 gap-px mb-1 sm:mb-2 flex-shrink-0">
              {diasSemana.map((dia) => (
            ...
            <div className="grid grid-cols-7 gap-px bg-border flex-1 overflow-hidden" style={{ gridAutoRows: '1fr' }}>
              {days.map((dayInfo, index) => {
                ...
                return (
                  <div
                    ...
                    className={`
                      bg-background p-0.5 sm:p-2 relative overflow-hidden flex flex-col cursor-pointer hover:bg-accent/50 transition-colors
                      ${!dayInfo.isCurrentMonth ? 'opacity-40' : ''}
                      ${isTodayDate ? 'ring-1 sm:ring-2 ring-primary ring-inset' : ''}
                    `}
                  >
                    <div className="flex items-center justify-between mb-0.5 sm:mb-1 flex-shrink-0">
                      <span className={`
                        text-[10px] sm:text-sm font-medium
                        ${isTodayDate ? 'bg-primary text-primary-foreground rounded-full w-4 h-4 sm:w-6 sm:h-6 flex items-center justify-center text-[10px] sm:text-sm' : ''}
                      `}>
                        {dayInfo.day}
                      </span>
                    </div>

                    <div className="space-y-0.5 sm:space-y-1 overflow-y-auto flex-1 min-h-0 scrollbar-hide">
                      {eventosDay.slice(0, 2).map((evento) => (
                        <div
                          key={evento.CODATIVIDADE}
                          className="text-[8px] sm:text-xs px-1 sm:px-2 py-0.5 sm:py-1 rounded text-white truncate hover:opacity-80 transition-opacity flex-shrink-0 flex items-center gap-0.5 sm:gap-1.5"
                          style={{ backgroundColor: evento.COR || '#22C55E' }}
                          title={`${evento.TITULO} - ${evento.DESCRICAO}`}
                        >
                          {evento.STATUS === 'REALIZADO' && <CheckCircle2 className="w-2 h-2 sm:w-3.5 sm:h-3.5 flex-shrink-0" />}
                          {evento.STATUS === 'ATRASADO' && <AlertCircle className="w-2 h-2 sm:w-3.5 sm:h-3.5 flex-shrink-0" />}
                          {evento.STATUS === 'AGUARDANDO' && <Clock className="w-2 h-2 sm:w-3.5 sm:h-3.5 flex-shrink-0" />}
                          <span className="truncate">{evento.TITULO}</span>
                        </div>
                      ))}
                      {eventosDay.length > 2 && (
                        <div className="text-[8px] sm:text-xs text-muted-foreground px-1 sm:px-2 flex-shrink-0">
                          +{eventosDay.length - 2}
                        </div>
                      )}
                    </div>
